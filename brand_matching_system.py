...
class BrandMatchingSystem:
    ...
    def load_data(self):
        ...
        try:
            ...
            for s in syns:
                ...
                self.synonym_rules.append({'std': s.standard_word.lower(), 'syn': s.synonym_word.lower(), 'scope': scope, 'exact': s.is_exact_match})
            ...
            for p in prods:
                ...
                row = {
                    '브랜드': p.brand, '상품명': p.product_name, '옵션입력': p.options, '중도매': p.wholesale_name, '공급가': p.supply_price,
                    '_p_norm': p_norm, '_db_colors': db_c_expanded, '_db_sizes': db_s_expanded,
                    '_db_colors_raw': raw_db_c, '_db_sizes_raw': raw_db_s
                }
                data.append(row)
    ...
    def convert_sheet1_to_sheet2(self, sheet1_df: pd.DataFrame) -> pd.DataFrame:
        sheet2_columns = ['A열(ㅇ)', 'B열(미등록주문)', 'C열(주문일)', 'D열(아이디주문번호)', 'E열(ㅇ)', 'F열(주문자명)', 'G열(위탁자명)', 'H열(브랜드)', 'I열(상품명)', 'J열(색상)', 'K열(사이즈)', 'L열(수량)', 'M열(옵션가)', 'N열(중도매명)', 'O열(도매가격)', 'P열(미송)', 'Q열(비고)', 'R열(이름)', 'S열(전화번호)', 'T열(주소)', 'U열(아이디)', 'V열(배송메세지)', 'W열(금액)']
        ...
        for i, (idx, row) in enumerate(sheet1_df.iterrows()):
            ...
            if len(sheet1_df.columns) >= 4:
                name, addr = str(row.iloc[3]), str(row.iloc[10]) if len(sheet1_df.columns) >= 11 else ""
                a3 = self.extract_third_word_from_address(addr)
                sheet2_row['G열(위탁자명)'] = f"{name}({a3})" if a3 else name
            ...
            if len(sheet1_df.columns) >= 6: sheet2_row['J열(색상)'], sheet2_row['K열(사이즈)'] = lo.parse_options(str(row.iloc[5]))
    ...
    # 🌟 가중치 인자 추가 (p_w: 상품명비율, o_w: 옵션보너스, b_w: 브랜드보너스)
    def match_row(self, b: str, p: str, s: str, c: str, p_w: float, o_w: float, b_w: float) -> Tuple:
        ...
        for rd in candidates:
            ...
            if p_sim >= 80:
                db_colors, db_sizes = rd.get('_db_colors', []), rd.get('_db_sizes', [])
        ...
        suggs = ls.get_4step_recommendations(p_norm, {b_clean}, self.db_records, up_c_norm, up_s_norm, c, s)
    ...
    def process_matching(self, sheet2_df: pd.DataFrame, weights: dict, progress_callback=None) -> Tuple[pd.DataFrame, List[Dict]]:
        ...
        for index, row in sheet2_df.iterrows():
            if progress_callback: progress_callback(index + 1, len(sheet2_df))
            b, p, s, c = str(row.get('H열(브랜드)', '')), str(row.get('I열(상품명)', '')), str(row.get('K열(사이즈)', '')), str(row.get('J열(색상)', ''))
            qty = row.get('L열(수량)', 1)
            ...
            if ok:
                ...
                failed_products.append({'발주_브랜드': b, '발주_상품명': p, '옵션': f"{c}/{s}", '💡추천_1순위': suggs[0] if len(suggs)>0 else "추천없음"})
